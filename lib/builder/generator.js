...
const { isUrl, promisifyRoute, waitFor, flatRoutes } = Utils
...
export default class Generator extends Tapable {
        ...
        generateRoutes = await promisifyRoute((callback) => {
          if (typeof this.options.generate.routes === 'function') {
            ...
            if (promise instanceof Promise && typeof promise.then === 'function') {
              return promise.then((routes) => { callback(null, routes) }).catch((e) => { callback(e) })
            } else {
              return callback(null, promise)
            }
          } else {
            return callback(null, this.options.generate.routes || [])
          }
        })
  ...
  async generateRoute ({ route, payload = {}, errors = [] }) {
    ...
    const fullPath = join(this.distPath, path)

    // Make sure the sub folders are created
    await mkdirp(dirname(fullPath))
    await writeFile(fullPath, html, 'utf8')

    await this.applyPluginsAsync('routeGenerated', { route, path })
