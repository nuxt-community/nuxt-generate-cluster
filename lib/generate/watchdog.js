...
export default class Watchdog extends Tapable {
  constructor () {
    super()
  ...
  async countAlive () {
    ...
    while ((worker = Iter.next()) && !worker.done) {
      if (typeof worker.value !== 'undefined') {
        let workerAlive = await this.applyPluginsWaterfall('isWorkerAlive', { worker: worker.value })
        if (workerAlive) {
          alive++
        }
      }
  ...
  async allDead () {
    ...
    while ((worker = Iter.next()) && !worker.done) {
      if (typeof worker.value !== 'undefined') {
        ...
        let isDead = this.workers[worker.value.id].duration > 0
        if (!isDead) {
          return false
        }
      }
